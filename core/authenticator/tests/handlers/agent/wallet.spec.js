...
const { toBase58, toDid } = require('@ocap/util');
...
describe('#WalletHandlers', () => {
  ...
  test('should handle common did-auth attach as expected', async () => {
    ...
    agentStorage.create(authorizeId, {
      ...
      appName: 'DID Wallet Demo',
      appDescription: 'Demo application to show the potential of DID Wallet',
    ...
    expect(authInfo1.iss).toEqual(`did:abt:${authorizer.address}`);
    expect(authInfo1.agentDid).toEqual(toDid(agent.address));
