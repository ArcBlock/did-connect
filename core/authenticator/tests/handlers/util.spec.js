const { toLower } = require('lodash');
const { parseWalletUA, getStepChallenge, preparePathname, prepareBaseUrl } = require('../../lib/handlers/util');

describe('#parseWalletUA', () => {
  describe('#android', () => {
    test('should parse "User-Agent: okhttp/3.12.2 (Linux; U; LGE Build/Nexus5)" correctly', async () => {
      const wallet = parseWalletUA('User-Agent: okhttp/3.12.2 (Linux; U; LGE Build/Nexus5)');
      expect(wallet).toEqual({ os: '', version: '', jwt: '1.0.0' });
    });

    test('should parse "User-Agent: okhttp/3.12.2 ArcWallet/ (Linux; U; Android 25; LGE Build/Nexus5)" correctly', async () => {
      const wallet = parseWalletUA('User-Agent: okhttp/3.12.2 ArcWallet/ (Linux; U; Android 25; LGE Build/Nexus5)');
      expect(wallet).toEqual({ os: 'android', version: '', jwt: '1.0.0' });
    });

    test('should parse "User-Agent: okhttp/3.12.2 ArcWallet/1.3.62 (Linux; U; Android 25; LGE Build/Nexus5)" correctly', async () => {
      const wallet = parseWalletUA(
        'User-Agent: okhttp/3.12.2 ArcWallet/1.3.62 (Linux; U; Android 25; LGE Build/Nexus5)'
      );
      expect(wallet).toEqual({ os: 'android', version: '1.3.62', jwt: '1.0.0' });
    });

    test('should parse "User-Agent: okhttp/3.12.2 ArcWallet/2.7.17 (Linux; U; Android 25; LGE Build/Nexus5)" correctly', async () => {
      const wallet = parseWalletUA(
        'User-Agent: okhttp/3.12.2 ArcWallet/2.7.17 (Linux; U; Android 25; LGE Build/Nexus5)'
      );
      expect(wallet).toEqual({ os: 'android', version: '2.7.17', jwt: '1.1.0' });
    });
  });

  describe('#ios', () => {
    test('should parse "iPhone12,3 iOS/13.0 CFNetwork/1098.7" correctly', async () => {
      const wallet = parseWalletUA('iPhone12,3 iOS/13.0 CFNetwork/1098.7');
      expect(wallet).toEqual({ os: '', version: '', jwt: '1.0.0' });
    });

    test('should parse "ArcWallet/ iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0" corcorrectlyrect', async () => {
      const wallet = parseWalletUA('ArcWallet/ iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0');
      expect(wallet).toEqual({ os: 'ios', version: '', jwt: '1.0.0' });
    });

    test('should parse "ArcWallet/1.3.29 iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0" correctly', async () => {
      const wallet = parseWalletUA('ArcWallet/1.3.29 iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0');
      expect(wallet).toEqual({ os: 'ios', version: '1.3.29', jwt: '1.0.0' });
    });

    test('should parse "ArcWallet/2.7.2 iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0" correctly', async () => {
      const wallet = parseWalletUA('ArcWallet/2.7.2 iPhone12,3 iOS/13.0 CFNetwork/1098.7 Darwin/19.0.0');
      expect(wallet).toEqual({ os: 'ios', version: '2.7.2', jwt: '1.1.0' });
    });
  });

  describe('#web', () => {
    test('should parse "ArcWallet/0.5.0"', async () => {
      const wallet = parseWalletUA('ArcWallet/0.5.0');
      expect(wallet).toEqual({ os: 'web', version: '0.5.0', jwt: '1.1.0' });
    });
  });
});

describe('#getStepChallenge', () => {
  test('should return random bytes', async () => {
    const c1 = getStepChallenge();
    const c2 = getStepChallenge();
    expect(c1).toBeTruthy();
    expect(c2).toBeTruthy();
    expect(c1).not.toEqual(c2);
  });
});

describe('#preparePathname', () => {
  test('should return correct path when not nested', async () => {
    const path = '/api/did/login/auth';
    const req = {
      originalUrl: '/api/did/login/auth',
    };

    expect(preparePathname(path, req)).toEqual('/api/did/login/auth');
  });

  test('should return correct path when not nested: retrieve', async () => {
    const path = '/api/did/login/retrieve';
    const req = {
      originalUrl: '/api/did/login/retrieve',
    };

    expect(preparePathname(path, req)).toEqual('/api/did/login/retrieve');
  });

  test('should return correct path when nested', async () => {
    const path = '/api/did/login/auth';
    const req = {
      originalUrl: '/root/nested/api/did/login/auth',
    };

    expect(preparePathname(path, req)).toEqual('/root/nested/api/did/login/auth');
  });

  test('should return correct path when netlify', async () => {
    const path = '/api/did/login/auth';
    const req = {
      originalUrl: '/.netlify/functions/app/api/did/login/auth',
    };

    expect(preparePathname(path, req)).toEqual('/.netlify/functions/app/api/did/login/auth');
  });
});

describe('repareBaseUrl', () => {
  describe('protocol, orders: params.x-real-protocol > header.X-Real-protocol > header.X-Forwarded-Proto > req.protocol ', () => {
    test('from params.x-real-protocol', () => {
      const req = {
        protocol: 'http',
        headers: {
          'x-real-hostname': 'real-hostname.com',
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req, { 'x-real-protocol': 'https' });
      expect(new URL(url).protocol).toEqual('https:');
    });

    test('from header.x-real-protocol', () => {
      const req = {
        protocol: 'http',
        headers: {
          'x-real-protocol': 'https',
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).protocol).toEqual('https:');
    });

    test('from header.X-Forwarded-Proto', () => {
      const req = {
        protocol: 'http',
        headers: {
          'x-forwarded-proto': 'https',
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).protocol).toEqual('https:');
    });

    test('from req.protocol', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).protocol).toEqual('http:');
    });
  });

  describe('hostname, orders: header.x-real-hostname > header.host', () => {
    test('from header.x-real-hostname', () => {
      const req = {
        protocol: 'http',
        headers: {
          'x-real-hostname': 'real-hostname.com',
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).hostname).toEqual('real-hostname.com');
    });

    test('from header.host', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[key];

      const url = prepareBaseUrl(req);
      expect(new URL(url).hostname).toEqual('host.com');
    });
  });

  describe('port, orders: params.x-real-port > header.X-Real-Port > host', () => {
    test('from params.x-real-port', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com:8083',
          'x-real-port': 8082,
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req, { 'x-real-port': 8081 });
      expect(new URL(url).port).toEqual('8081');
    });

    test('from header.X-Real-Port', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com:8083',
          'x-real-port': 8082,
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).port).toEqual('8082');
    });

    test('from host', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com:8083',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).port).toEqual('8083');
    });

    test('should return empty string if the port is 80', () => {
      const req = {
        protocol: 'http',
        headers: {
          host: 'host.com',
        },
      };

      req.get = (key) => req.headers[toLower(key)];

      const url = prepareBaseUrl(req);
      expect(new URL(url).port).toEqual('');
    });
  });
});
